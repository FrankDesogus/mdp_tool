...
class GridPageDiag:
    ...
    inherited_columns_from_previous_page: bool = False
    header_recovery_strategy: str = ""
    vertical_lines_strategy: str = ""
    footer_cutoff_applied: bool = False
...
def _extract_lines_from_grid_layout(pdf: pdfplumber.PDF) -> Tuple[List[Dict[str, Any]], List[str], Dict[str, Any]]:
    ...
    prev_header_y1: Optional[float] = None

    for page_idx, page in enumerate(pdf.pages):
        pd = GridPageDiag(page_number=page_idx + 1)
        pd.continuation_page_detected = page_idx > 0

        detected_vlines = _extract_vertical_grid_lines(page, _GRID_TABLE_Y_MIN, _GRID_TABLE_Y_MAX)
        hlines = [ln for ln in (page.lines or []) if abs(float(ln.get("y0", 0.0)) - float(ln.get("y1", 0.0))) < 1.0]
        pd.vertical_lines_count = len(detected_vlines)
        pd.horizontal_lines_count = len(hlines)

        if len(detected_vlines) >= _PDF_POLICY.min_vertical_lines_soft:
            vlines = detected_vlines
            pd.vertical_lines_strategy = "detected"
        elif pd.continuation_page_detected and prev_vlines:
            vlines = prev_vlines
            pd.vertical_lines_strategy = "inherited_from_previous_page"
            warnings.append(f"[grid] page {page_idx+1}: linee verticali insufficienti ({len(detected_vlines)}), riuso griglia pagina precedente.")
        else:
            pd.page_skipped_reason = f"low_vertical_lines:{len(detected_vlines)}"
            pd.vertical_lines_strategy = "insufficient_no_template"
            warnings.append(f"[grid] page {page_idx+1}: saltata (linee verticali insufficienti: {len(detected_vlines)}).")
        ...
        if hb:
            ...
            pd.header_recovery_strategy = "detected"
            hy0, hy1 = hb
            header_words = [w for w in usable_words if hy0 <= float(w.get("top", 0.0)) <= hy1]
            col_map = _build_grid_col_map(header_words, vlines)
        elif pd.continuation_page_detected and prev_col_map:
            pd.found_header = False
            pd.inherited_columns_from_previous_page = True
            pd.header_recovery_strategy = "inherited_columns"
            col_map = dict(prev_col_map)
            pos_words = [w for w in usable_words if _is_pos_token(clean_pdf_text(w.get("text"))) and float(w.get("x0", 9999.0)) <= 110.0]
            if pos_words:
                hy1 = max(0.0, min(float(w.get("top", 0.0)) for w in pos_words) - 3.0)
            elif prev_header_y1 is not None:
                hy1 = prev_header_y1
            else:
                hy1 = _GRID_TABLE_Y_MIN
            warnings.append(f"[grid] page {page_idx+1}: header non trovato, uso template colonne pagina precedente.")
        else:
            pd.page_skipped_reason = "missing_header_no_template"
            pd.header_recovery_strategy = "failed"
        ...
        pd.footer_cutoff_applied = True
        hy1_effective = hy1 if hy1 is not None else _GRID_TABLE_Y_MIN
        body_words = [w for w in usable_words if float(w.get("top", 0.0)) > hy1_effective and float(w.get("top", 0.0)) < body_cutoff]
        ...
        merge_streak = 0
        pos_idx = col_map.get("pos", -1)
        merge_allowed_idx = {col_map[k] for k in ("desc", "notes", "manufacturer", "manufacturer_code", "type") if k in col_map}
...
def parse_bom_pdf_raw(path: Path) -> dict:
    ...
    with pdfplumber.open(path) as pdf:
        ...
        if should_try_layout:
            ...
            grid_header_failures = sum(1 for p in page_diags if str(p.get("header_recovery_strategy", "")) in {"failed"})
            grid_continuation_missing_header = sum(1 for p in page_diags if p.get("continuation_page_detected") and not p.get("found_header") and not p.get("inherited_columns_from_previous_page"))
            grid_score = _compute_parser_quality(grid_lines, grid_warn, pages_skipped=grid_pages_skipped)
            grid_score -= (_PDF_POLICY.parser_suspicious_penalty * float(grid_header_failures + grid_continuation_missing_header))

            prefer_grid = False
            reason = "tables_preferred"
            reason_detailed = "tables baseline retained"
            if grid_lines and not lines:
                prefer_grid = True
                reason = "grid_has_rows_tables_empty"
                reason_detailed = f"tables rows=0, grid rows={len(grid_lines)}"
            elif grid_lines and lines:
                prefer_grid = grid_score >= base_score
                reason = "quality_score_compare"
                reason_detailed = (
                    f"grid_score={grid_score:.2f} vs tables_score={base_score:.2f}; "
                    f"grid_skipped_pages={grid_pages_skipped}; grid_header_failures={grid_header_failures}"
                )
            ...
            parser_compare = {
                ...
                "selected_reason_detailed": reason_detailed,
                "grid_penalties": {
                    "pages_skipped": grid_pages_skipped,
                    "header_failures": grid_header_failures,
                    "continuation_missing_header": grid_continuation_missing_header,
                },
            }
    ...
    parse_partial_reasons: List[str] = []
    if severe_warnings:
        parse_partial_reasons.append(f"severe_warnings:{len(severe_warnings)}")
    if pages_skipped:
        parse_partial_reasons.append(f"pages_skipped:{pages_skipped}")
    if any(p.get("continuation_page_detected") and p.get("page_skipped_reason") for p in page_diags):
        parse_partial_reasons.append("continuation_pages_skipped")
    if parser_compare:
    ...
    summary = {
        ...
        "parser_selection_reason_detailed": parser_compare.get("selected_reason_detailed", "") if parser_compare else "",
        "parse_partial_reasons": parse_partial_reasons,
    }
    ...
    diagnostics_path = str((Path("diagnostics") / "pdf_debug" / f"{path.stem}.json").as_posix())
    ...
    meta = {
        ...
        "diagnostics_enabled": _PDF_POLICY.diagnostics_enabled,
    }
